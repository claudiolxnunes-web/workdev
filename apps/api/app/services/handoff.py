...
def create_plan(db: Session, data: dict[str, Any]) -> ExecutionPlan:
    ...
    ).filter(ExecutionPlan.backlog_id == task.id).scalar() + 1
    plan = ExecutionPlan(
        backlog_id=task.id,
        version=version,
        status="draft",
...
def approve_plan(db: Session, plan: ExecutionPlan) -> ExecutionPlan:
    if plan.status not in PLAN_EDITABLE:
        raise HandoffError(f"Plano não pode ser aprovado no estado {plan.status}")
    if not plan.acceptance_criteria:
        raise HandoffError("Inclua pelo menos um critério de aceite antes de aprovar")
    if not plan.validation_steps:
        raise HandoffError("Inclua pelo menos uma etapa de validação antes de aprovar")
...
def queue_build(db: Session, plan: ExecutionPlan, agent: str) -> tuple[AgentRun, AgentRunEvent]:
    ...
    if active:
        raise HandoffError(f"A task já possui uma execução ativa ({active.status})")
    run = AgentRun(
        plan_id=plan.id,
    ...
    db.add(run)
    db.flush()
    event = add_run_event(db, run, "build.queued", f"Build enviado para {agent}")
...
def update_run(
    ...
    if next_status and next_status != run.status:
        ...
        task = db.query(BacklogItem).filter(BacklogItem.id == run.backlog_id).first()
        if task:
            if next_status in {"running", "review"}:
                ...
                task.status = "done"
            elif next_status == "cancelled" and task.status != "done":
                task.status = "todo"
        ...
        if next_status == "running":
            (db.query(BacklogSubtask)
             ...
             .update({"assigned_agent": run.agent}, synchronize_session=False))

    for field in (
        "summary", "result", "error", "branch", "commit_sha", "deployment_url"
    ):
        if field in data and data[field] is not None:
            setattr(run, field, data[field])
    run.updated_at = _now()
    if not event and message:
        event = add_run_event(db, run, "build.progress", message)
    db.commit()
    db.refresh(run)
    if event:
        db.refresh(event)
...
def build_context(db: Session, run: AgentRun) -> dict[str, Any]:
    ...
    if not plan or not task:
        raise HandoffError("Plano ou task da execução não encontrado")
    project = db.query(Project).filter(Project.id == task.project_id).first()
    if not project:
        raise HandoffError("Projeto da execução não encontrado")
    subtasks = (db.query(BacklogSubtask)
                .filter(BacklogSubtask.backlog_id == task.id)
                .order_by(BacklogSubtask.execution_order.asc()).all())
    adrs = (db.query(ADR)
            .filter(
                ADR.project_id == project.id,
                or_(ADR.feature_id == task.id, ADR.feature_id.is_(None)),
            )
            .order_by(ADR.created_at.desc()).limit(20).all())
    knowledge = (db.query(KnowledgeEntry)
                 .filter(
                     or_(
    ...
    context = {
        "run": {
            "id": str(run.id), "agent": run.agent, "status": run.status,
            "summary": run.summary, "result": run.result, "error": run.error,
        ...
        "project": {
            ...
            "description": project.description, "stack": project.stack,
            "github_url": project.github_url, "dev_branch": project.dev_branch,
            "prod_branch": project.prod_branch, "vps": project.vps,
        },
        "task": {
        ...
        "subtasks": [
            {"id": str(row.id), "order": row.execution_order,
             "title": row.title, "description": row.description,
             "status": row.status, "result": row.result}
            for row in subtasks
        ],
        "adrs": [
            {"id": str(row.id), "title": row.title, "context": row.context,
             "decision": row.decision, "consequences": row.consequences,
             "status": row.status}
            for row in adrs
        ],
        "knowledge": [
        ...
        "events": [
            {"id": str(row.id), "type": row.event_type,
             "message": row.message, "payload": row.payload,
...
def _bullets(values: list[str]) -> str:
    return "\n".join(f"- {value}" for value in values) or "- Não informado"
...
def render_agent_prompt(context: dict[str, Any]) -> str:
    ...
    subtask_text = "\n".join(
        f"- [{row['status']}] {row['order']}. {row['title']}"
        for row in subtasks
